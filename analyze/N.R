...
plotN<-function(species,river,error=F){
  ...
  if(r<3){ylimit<-c(0,320)
  } else {if(r==3){ylimit<-c(0,600)
  } else {ylimit<-c(0,1200)
  }}
    ...
    for(r in rivers){
      if(sp=='Bnt'&r=='Mitchell'){
        plot(NA,xlim=c(0,1),ylim=c(0,1),
             axes=F,xlab="",ylab="")
        legend(0.35,0.65,c("Random Year","Extreme + Stock Recruit",
                           "Mean + Stock Recruit","PCA + Stock Recruit"),lty=1,
