...
start<-c(199,41,137,155)
days<-matrix(c(199:(199+45),
               41:(41+45),
               137:(137+45),
               155:(155+45)),ncol=4)
thresh<-c(0.92,0.95,NA,0.98)

e1<-env.cov("discharge",threshold=thresh[1],
               high.low=ifelse(thresh[1]>0.5,"high","low"),
               freq.dur="sumAbove",days=days[,1])

for(bla in c(2,4)){
e1[,bla]<-env.cov("discharge",threshold=thresh[bla],
                high.low=ifelse(thresh[bla]>0.5,"high","low"),
                freq.dur="sumAbove",days=days[,bla])[,bla]
}
e1[,3]<-env.cov("discharge",days=days[,3],FUN=mean)[,3]

for(dur in c(45)){
duration<-dur
startDays<-seq(1,366-duration,5)
...
for(riv in rivers){
  ...
  for(start in startDays){
    ...
    for(thresh in thresholds){
      ...
      a<-lm(yoy~e[,column]+e1[,column]+adults)
      ...
      p[which(start==startDays),which(thresh==thresholds),column]<-summary(a)$coefficients[14]
    }
    meanE<-env.cov("discharge",days=days,FUN=mean)
    meanModel<-lm(yoy~meanE[,column]+e1[,column]+adults)
    rsqMean[which(start==startDays),column]<-summary(meanModel)$r.squared
    aicMean[which(start==startDays),column]<-AICc(meanModel)
    pValueMean[which(start==startDays),column]<-summary(meanModel)$coefficients[14]
...
assign(paste0("pValueMean",dur),rsqMean)
...
toSave<-c(ls()[grepl("rsq",ls())],ls()[grepl("aic",ls())],ls()[grepl("slope",ls())],
          ls()[grepl("pValue",ls())])
save(list=toSave,file="results/thresholdResults.rdata")
