postCor<-function(x,vars=c('beta','c'),sp=1,modelName){
  require(reshape2)
  require(data.table)
  for(v in vars){
    assign(v,
      data.table(melt(x[[v]][,,,sp])))
    get(v)[,Var3:=paste0(v,Var3)]
  }
  ...
  data<-data.table(dcast(data,Var1+Var2~Var3))
  setnames(data,c("Var1","Var2"),c("iter","river"))
  
  corMatrix<-cor(data[,3:ncol(data),with=F])
  
  corByRiver<-array(NA,dim=c(ncol(data)-2,ncol(data)-2,4))
  dimnames(corByRiver)<-list(names(data)[3:ncol(data)],
